...
3. **不做文首署名标签**。开头不出现「李云龙主笔」这类橙底标签，
   署名只在文末出现一次（组件 14）。
4. **不做正文关键词下划线**。本主题取消「标记层」，正文不加任何下划线标记。
   这一条**显式覆盖 SKILL.md 第 3 条**（那里把「每段 1–3 处关键词下划线」
   定为核心特色、要求逐段落实）——用本主题时不执行那条，
   `theme-index.md` 的「正文下划线 CSS」列对本主题也不适用。
...
| 强调色（橙） | `#FF6B35` | 重点句加粗、数据标注、一级标题竖条 |
...
一个章节内橙色加粗不超过 2 处。取消下划线之后橙色出现频次本就低，别再往回加。

> 去掉文首署名标签后，橙色不再有任何实心色块的用法，
> 只剩下：一级标题竖条、重点加粗文字、数据标注、无序列表圆点、
...
数据标注不计入 6a 的 5 处配额，但同一段里别超过 3 个，否则页面像仪表盘。

### 6c. 蓝色概念标签（核心术语，每篇 2~4 个）
...
数据卡片只在原文没有把数字写进句子时用；原文已经写了的，用 6b 行内标注，别重复一遍。
...
| 层 | 手段 | 配额 |
|---|---|---|
| 锚点层 | 组件 6a 橙色加粗 | 全文 ≤ 5 处 |
| 结构层 | 组件 4 / 4b 标题、组件 3 分割线、组件 7 引用块 | 按结构需要 |

数据标注（6b）独立于锚点层配额，但每段 ≤ 3 个。

**取消标记层的代价，装配时要主动补偿**：原本每段 1–3 处下划线是全文密度最高的
视觉落点，没有它之后，长段落会变成没有抓手的整块文字，手机上容易失焦。
补偿手段只有三个，都在结构层：把过长的段落交给引用块（组件 7）承载、
用二级标题（组件 4b）切分、以及保证章节之间的分割线和 48px 留白不被压缩。
**不要用加粗去补**——锚点层 5 处的配额是硬上限，补到第 6 处就失去锚点意义了。
...
| 商业观点 / 评论 | 5 + 7b + 7c | 7a 引用、6c 概念标签 | 4 章以上带编号，否则不带 |
| 数据驱动分析 | 5 + 10a + 10b 表格 | 8a 结论条、6b 数据标注 | 带编号 |
...
| 普通段落 | 组件 5 | 纯文字，不加下划线 |
| `**加粗**` | 组件 6a | 橙色加粗，全文 ≤ 5 处 |
| 含数字的指标 | 组件 6b | 数字带单位一起标橙 |
| `> 引用` | 组件 7a | 浅底 + 蓝色左条 |
...
| `` `代码` `` | 组件 6d | 行内代码 |
| `![]()` | 组件 12 | 图片容器 |
| `<u>文字</u>` / `++文字++` | 组件 6a | 原文自带的下划线标记，转成橙色加粗，计入 5 处配额；超配额则按普通文字处理 |
| 文首作者信息 | 删除 | 不生成任何开头署名标签 |
| 文末署名 | 组件 14 | 统一成「—— 李云龙增长研习社」 |
| —— | —— | **不生成导读卡、不生成英文标签、不生成文首署名、不生成下划线** |
...
- **不用彩色底块装文字**：6a 重点加粗是纯橙字、不加底色；引言卡高亮也是橙字不是橙底。
